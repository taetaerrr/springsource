...
public class QueryDslOrderRepositoryImpl extends QuerydslRepositorySupport implements QueryDslOrderRepository {
    ...
    public List<Object[]> joinTest() {
        ...
        // .join(qMember).on(qOrder.member.eq(qMember)).select(qOrder, qMember);
    ...
    public List<Object[]> subQueryTest() {
        ...
        JPQLQuery<Order> tuple = from(qOrder)
                .join(qMember).on(qOrder.member.eq(qMember));

        // select에 사용할 서브쿼리 생성
        JPAExpressions.select(qOrderItem.order.count()).from(qOrderItem).where(qOrderItem.order.eq(qOrder)).groupBy(qOrderItem.order)
    }
