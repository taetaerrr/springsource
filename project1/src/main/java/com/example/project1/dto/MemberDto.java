...
public class MemberDto {
    ...
    @Pattern(regexp = "(?=.*[A-Za-z])(?=.*[0-9])(?=.*[!@#])[A-Za-z0-9!@#]{6,12}$", message = "아이디는 영어대소문자, 숫자, 특수문자(!@#)를 사용해서 6~12자리로 작성해야합니다.")
    ...
    // @Min,@Max 와 @NotEmpty, @NotBlank는 같이 사용 불가
    @NotNull(message = "나이는 필수 요소입니다.")
    @Min(value = 0, message = "최소 0 이상이어야 합니다.")
    @Max(value = 0, message = "최대 120 이하이어야 합니다.")
    private Integer age;
