  ...
  fetch(`/replies/board/${bno}`)
    ...
    .then((data) => {
      ...
      data.forEach((reply) => {
        ...
        result +=
          '<div class="p-3"><img src="" alt="" class="rounded-circle mx-auto d-block" /> </div>';
        result += ' <div class="flex-grow-1 align-self-center">';
        result += "<span>${reply.replyer}</span>";
        result += '<div><span class="fs-5">${reply.text}</span></div>';
        result +=
          '<div class="text-muted"><span class="small">${reply.regDate}</span></div></div>';
        result +=
          '<div class="d-flex flex-column align-self-center"><div class="mb-2">';
        result +=
          '<button class="btn btn-outline-danger btn-sm">삭제</button></div>';
        result +=
          ' <div><button class="btn btn-outline-success btn-sm">수정</button></div>';
        result += "</div></div>";
      });
